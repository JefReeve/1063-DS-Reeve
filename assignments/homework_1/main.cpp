...
int loadClassList(Student *classList, string filename)
{
	...
	while (!fin.eof())
	{
		// Read the first and last name of the student as well as the number
		// of recorded grades which is saved to the first index of the array
		fin >> classList[i].fname >> classList[i].lname >> classList[i].grades[0];
		
		// Reads in grades until the number of grades listed or 10 grades
		for (int j = 1; j - 1 < classList[i].grades[0] && j < 11; j++)
		{
...
void printClassList(Student *classList, int classSize)
{
	...
	{
		// print the first and last name of the student
		cout << left << setw(10) << classList[i].fname << " "
			<< setw(10) << classList[i].lname << ": ";
		
		// print the amount of grades saved in the first index of the array
		for (int j = 1; j - 1 < classList[i].grades[0] && j < 11; j++)
		{
			cout << setw(3) << classList[i].grades[j] << " ";
		}
		cout << endl;
	}
}
